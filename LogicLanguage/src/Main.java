            ...
            public void keyPressed(java.awt.event.KeyEvent e) {
                if (e.getKeyCode() == java.awt.event.KeyEvent.VK_TAB) {
                    ...
                    try {
                        ...
                        // Calculate indentation level based on braces before cursor
                        String beforeCursor = text.substring(0, caret);
                        int indentLevel = 0;
                        ...
                        // Check if current line ends with { (don't add extra indent yet)
                        String trimmed = currentLine.trim();
                        boolean lineEndsWithBrace = trimmed.endsWith("{");
                        
                        // If we're about to close a brace, dedent
                        String afterCursor = text.substring(caret);
                        String nextChar = afterCursor.isEmpty() ? "" : afterCursor.substring(0, 1).trim();
                        if (nextChar.equals("}")) {
                            indentLevel--;
                        }
                        ...
                        doc.insertString(caret, indent.toString(), null);
                    } catch (Exception ignored) {}
                }
            }
        });
